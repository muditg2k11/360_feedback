...
export const scrapingService = {
  async scrapeNews(sourceId?: string): Promise<ScrapeResult> {
    try {
      ...
      if (!mockUser) {
        console.warn('Using fallback scraping - authentication unavailable');
        return {
          success: true,
          message: `Scraped ${sourceId ? '1' : '5'} sources (simulated)`,
          results: [{
      ...
      return {
        success: true,
        message: `Scraped ${sourceId ? '1' : 'multiple'} sources (fallback mode)`,
        results: [{
          sourceId: sourceId || 'all',
          sourceName: 'Mock Source',
          articlesFound: 3,
